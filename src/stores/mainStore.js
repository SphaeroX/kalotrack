...
import { getCurrentDate, calculateCalories, calculateDietCalories, calculateNutrientDistribution } from '@/utils/main.js';
...
export const useMainStore = defineStore('main', {
    ...
    actions: {
        ...
        getSuggestions() {
            this.kcalSuggested = JSON.parse(localStorage.getItem("kcalSuggested")) || 2000;
            this.nutrientSuggested = JSON.parse(localStorage.getItem("nutrientsSuggested")) || false;
        },
        ...
        updateBodyData() {
            ...
            localStorage.setItem('nutrientsSuggested', JSON.stringify(this.nutrientSuggested));
        },
    ...
    getters: {
        carbsConsumed(state) {
            return state.items.reduce((total, item) => total + (item.carbohydrates || 0), 0);
        },
        proteinConsumed(state) {
            return state.items.reduce((total, item) => total + (item.protein || 0), 0);
        },
        fatConsumed(state) {
